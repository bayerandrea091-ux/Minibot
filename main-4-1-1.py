...
def _group_set_cooldown(chat_id, retry_after):
    ...
    print(f"[FLOOD] Group {chat_id} cooldown {retry_after}s")
        ...
        try:
            ...
            if '429' in err:
                ...
                if _flood_callback:
                    ...
                    # For queue worker: don't sleep, just return None and requeue
                    return None
            else:
...
def _send_queue_worker():
    ...
    while True:
        ...
        while True:
            with _send_queue_lock:
                if not _send_queue:
                    ...
                    break
                priority, seq, chat_id, text, result_holder = heapq.heappop(_send_queue)

            if not _group_is_allowed(chat_id):
                # Group in cooldown or rate-limited — push to back of queue
                requeue_batch.append((priority + 0.001, seq, chat_id, text, result_holder))
                time.sleep(0.05)
                continue
            ...
            if sent is None and not _group_is_allowed(chat_id):
                # Just got 429 — requeue this message for later
                requeue_batch.append((priority + 0.001, seq, chat_id, text, result_holder))
            else:
        ...
        if requeue_batch:
            ...
            time.sleep(1)  # brief pause before retrying cooled-down groups

_send_worker_thread = threading.Thread(target=_send_queue_worker, daemon=True)
_send_worker_thread.start()

def _enqueue(chat_id, text, result_holder, priority=3):
    global _send_queue_seq
    with _send_queue_lock:
        _send_queue_seq += 1
        heapq.heappush(_send_queue, (priority, _send_queue_seq, chat_id, text, result_holder))
        _send_queue_event.set()

def safe_send(chat_id, text, priority=3):
    """Queue a message, block until sent. Returns Message or None."""
    result_holder = []
    _enqueue(chat_id, text, result_holder, priority)
...
def safe_send_nowait(chat_id, text, priority=2):
    """Queue a message without blocking. Fire and forget."""
    _enqueue(chat_id, text, None, priority)
