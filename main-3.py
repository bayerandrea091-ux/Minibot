...
# ── Broadcast lock ────────────────────────────────────────────────────────────
# Prevents the global repeat thread and manual broadcast from sending to the
# same groups at the same time, which caused duplicate messages and 429 errors.
_broadcast_lock = threading.Lock()

# ── Rate-limit-safe send helper ───────────────────────────────────────────────
# Adds a small delay between every send and handles Telegram 429 errors by
# waiting the required retry_after time and trying once more automatically.
_INTER_SEND_DELAY = 0.07   # 70 ms between sends ≈ safe under Telegram's limit

def safe_send_message(chat_id, text):
    """Send a message with inter-send delay and automatic 429 retry.
    Returns the sent Message object or raises on non-429 errors."""
    ...
    try:
        ...
        err = str(e)
        if '429' in err:
            # Parse retry_after from Telegram's error description
            retry_after = 30  # safe default
            try:
                retry_after = int(err.split('retry after')[-1].strip().split()[0])
            except Exception:
                pass
            time.sleep(retry_after + 1)
            return bot.send_message(chat_id, text)  # one retry
        raise  # re-raise non-429 errors so callers can handle them
...
def repeat_message_task(chat_id):
    while r.get(f'repeat_task:{chat_id}') == 'True':
        ...
        try:
            ...
            sent = safe_send_message(chat_id, text)
...
def show_main_menu(chat_id, text, message_id=None):
    ...
    markup.add(
        ...
        types.InlineKeyboardButton("📊 Updates", callback_data="updates_menu"),
    )
    if message_id:
        try:
...
def callback(call):
    ...
    # ── UPDATES MENU ──────────────────────────────────────────────────────────
        ...
        bot.answer_callback_query(call.id, "🔄 Checking groups..." if data == "updates_refresh" else "")
        groups = list(get_groups())
        active = []
        api_errors = []
        permission_errors = []
        ...
        for g in groups:
            try:
                chat = bot.get_chat(g)
                title = chat.title or f"Group {g}"
                r.set(f'cache_group_title:{g}', title, ex=600)
                try:
                    member = bot.get_chat_member(g, bot.get_me().id)
                    if member.status in ['kicked', 'left']:
                        permission_errors.append((g, title, "Removed/Kicked"))
                        remove_group(g)
                    elif member.status == 'restricted':
                        permission_errors.append((g, title, "Restricted"))
                    else:
                        status_str = "Admin" if member.status in ['administrator', 'creator'] else "Member"
                        r.set(f'cache_group_status:{g}', status_str, ex=600)
                        active.append((g, title, status_str))
                except Exception as e2:
                    permission_errors.append((g, title, f"Permission err: {str(e2)[:40]}"))
            except telebot.apihelper.ApiTelegramException as e:
                err = str(e)
                if '429' in err:
                    # Rate limited — count as active (unknown), don't remove
                    cached_title = r.get(f'cache_group_title:{g}') or f"Group {g}"
                    api_errors.append((g, cached_title, "Rate limited (429)"))
                else:
                    cached_title = r.get(f'cache_group_title:{g}') or f"Group {g}"
                    api_errors.append((g, cached_title, f"API err {err[:40]}"))
            except Exception as e:
                cached_title = r.get(f'cache_group_title:{g}') or f"Group {g}"
                api_errors.append((g, cached_title, str(e)[:40]))

        lines = [f"📊 *Group Updates*\n"]
        lines.append(f"✅ *Active groups: {len(active)}*")
        for _, title, status in active[:30]:
            lines.append(f"  • {title} ({status})")
        if len(active) > 30:
            lines.append(f"  ...and {len(active) - 30} more")

        lines.append(f"\n⚠️ *API Error groups: {len(api_errors)}*")
        for _, title, reason in api_errors[:15]:
            lines.append(f"  • {title} — {reason}")

        lines.append(f"\n🚫 *Removed / Permission errors: {len(permission_errors)}*")
        for _, title, reason in permission_errors[:15]:
            lines.append(f"  • {title} — {reason}")
        ...
        try:
            bot.edit_message_text(msg_text, cid, mid, reply_markup=markup, parse_mode='Markdown')
        except Exception:
            try:
                bot.edit_message_text(msg_text.replace('*', ''), cid, mid, reply_markup=markup)
            except Exception:
                pass
...
def process_broadcast_all(message):
    ...
    with _broadcast_lock:
        for group in groups:
            try:
                sent = safe_send_message(group, text)
