...
def start_repeat_thread(chat_id):
    ...
    if r.get(key) == 'True':
        if chat_id in active_repeat_threads:
            # already running
            pass
        else:
...
def callback(call):
    ...
    if call.data == "set_global_start_reply":
        bot.edit_message_text("Send new custom reply for private /start.\nSend 'reset' to remove.", cid, mid)
        ...
        bot.edit_message_text("Send new custom reply for /start@AllMusicShazamandlyrics_bot in group.\n'reset' to remove.", cid, mid)
        bot.register_next_step_handler(call.message, lambda m: process_group_start_reply(m, chat_id))
        bot.answer_callback_query(call.id)

    elif call.data == "broadcast_all":
        bot.edit_message_text("Send message to broadcast to all groups.", cid, mid)
        ...
        text = "Your Groups:" if call.data == "my_groups" else "Select group to send:"
        bot.edit_message_text(text, cid, mid, reply_markup=markup)
        bot.answer_callback_query(call.id)
        ...
        for g in groups:
            try:
                ...
                if "chat not found" in str(e).lower() or "forbidden" in str(e).lower():
                    remove_group(g)
                    removed += 1
        bot.answer_callback_query(call.id, f"Refreshed. Removed {removed} invalid.")
        ...
        text = f"Group: {title}\nStatus: {status}\nLink-only: {'ON' if link_only_this else 'OFF'}\nRepeating: {'ON' if repeat_on else 'OFF'}\n\nActions:"

        markup = types.InlineKeyboardMarkup(row_width=1)
        markup.add(types.InlineKeyboardButton("Send Message", callback_data=f"send_to_group:{chat_id}"))
        markup.add(types.InlineKeyboardButton("Setup Repeating", callback_data=f"setup_repeat:{chat_id}"))
        if is_admin and r.get(f'last_sent:{chat_id}'):
            markup.add(types.InlineKeyboardButton("Pin Last", callback_data=f"pin_last:{chat_id}"))
        if get_sent_messages(chat_id):
            markup.add(types.InlineKeyboardButton("Purge My Messages", callback_data=f"purge:{chat_id}"))
        markup.add(types.InlineKeyboardButton("Set /start@ Reply", callback_data=f"set_group_start_reply:{chat_id}"))
        markup.add(types.InlineKeyboardButton(f"{'Disable' if link_only_this else 'Enable'} Link-Only", callback_data=f"toggle_group:{chat_id}"))
        ...
        text = f"Repeating setup (group {chat_id})\nStatus: {'ON' if repeat_on else 'OFF'}\nInterval: {interval}s\nAuto-delete prev: {'ON' if autodelete else 'OFF'}\n\nOptions:"
        ...
        markup.add(types.InlineKeyboardButton(f"Auto-del prev: {'ON' if autodelete else 'OFF'}", callback_data=f"toggle_autodelete:{chat_id}"))
    ...
    elif call.data.startswith("set_interval_sec:") or call.data.startswith("set_interval_min:"):
        _, chat_id_str = call.data.split(":", 1)
        chat_id = int(chat_id_str)
        unit = "sec" if "sec" in call.data else "min"
        bot.edit_message_text(f"Send interval in {unit} (number only)", cid, mid)
        ...
        bot.answer_callback_query(call.id, f"Auto-delete prev now {'OFF' if current else 'ON'}")
        callback(types.CallbackQuery(id=call.id, from_user=call.from_user, message=call.message, data=f"setup_repeat:{chat_id}"))
        ...
        bot.answer_callback_query(call.id, f"Purged {deleted} msgs (old ones may remain)")
        ...
        _, chat_id_str = call.data.split(":", 1)
        chat_id = int(chat_id_str)
        bot.edit_message_text("Send your message now (will be sent + pin offer if admin)", cid, mid)
        ...
        if not msg_id:
            bot.answer_callback_query(call.id, "No last msg", show_alert=True)
            return
        try:
            bot.pin_chat_message(chat_id, int(msg_id))
            bot.answer_callback_query(call.id, "Pinned!")
        except Exception as e:
            bot.answer_callback_query(call.id, f"Pin failed: {str(e)}", show_alert=True)
        ...
        status = "OFF" if current else "ON"
        bot.answer_callback_query(call.id, f"Link-only now {status}")
        callback(types.CallbackQuery(id=call.id, from_user=call.from_user, message=call.message, data=f"group_menu:{chat_id}"))
...
def process_interval(message, chat_id, unit):
    ...
    callback(types.CallbackQuery(id="dummy", from_user=message.from_user, message=message, data=f"setup_repeat:{chat_id}"))
...
def process_broadcast_all(message):
    ...
    groups = get_groups()
    if not groups:
        bot.send_message(message.chat.id, "No groups.")
        show_main_menu(message.chat.id, "Choose action:")
        return
    sent_count = 0
    for group in groups:
    ...
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("Go Back", callback_data="back"))
    bot.send_message(message.chat.id, f"Broadcast sent to {sent_count} groups.", reply_markup=markup)
    show_main_menu(message.chat.id, "Choose action:")
...
def process_single_message(message, group_id):
    ...
    try:
        ...
        bot.send_message(message.chat.id, "Sent.", reply_markup=markup)
        # For repeating: if repeating is on and no repeat_text set yet, use this as repeating text
        if r.get(f'repeat_task:{group_id}') == 'True' and not r.get(f'repeat_text:{group_id}'):
            r.set(f'repeat_text:{group_id}', text)
            bot.send_message(message.chat.id, "This message is now the repeating one (since no prior set).")
    except Exception as e:
...
if __name__ == '__main__':
    ...
    # Restart any active repeat tasks
    for g in get_groups():
