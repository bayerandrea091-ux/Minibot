...
def repeat_message_task(chat_id):
    while r.get(f'repeat_task:{chat_id}') == 'True':
        ...
        # Use safe_send so we get the sent message back (needed for self-delete + last_sent tracking)
        # But wrap in a thread so the repeat thread never blocks indefinitely
        result_holder = []
        with _send_queue_lock:
            _send_queue.append((chat_id, text, result_holder))
            _send_queue_event.set()

        # Wait up to 30s for this single message (much safer than 120s)
        ...
        sent = result_holder[0] if result_holder else None
        if sent:
...
def global_repeat_task():
    while r.get('global_repeat_task') == 'True':
        ...
        for chat_id in groups:
            ...
            # Queue without blocking — global repeat doesn't need to wait per group
            # We use nowait so the global repeat loop moves quickly through all groups
            # without tying up the send queue or blocking other operations
            if self_delete_after:
                # Need the sent message ID for self-delete — use short-wait send
                result_holder = []
                with _send_queue_lock:
                    _send_queue.append((chat_id, text, result_holder))
                    _send_queue_event.set()
                ...
                # No self-delete needed — fire and forget
...
def process_broadcast_all(message):
    ...
    # Acknowledge immediately so the bot doesn't freeze
    bot.send_message(
        message.chat.id,
        f"📢 Broadcast started to {len(groups)} group(s).\n"
        f"⏳ Sending at ~1 msg/sec — you'll get a report when done.",
        reply_markup=_back_markup("back")
    )
