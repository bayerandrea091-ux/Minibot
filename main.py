...
def handle_new_chat_members(message):
    for member in message.new_chat_members:
        if member.id == bot.get_me().id:
            ...
            # Try to gather detailed info
            try:
                chat = bot.get_chat(chat_id)
                title = chat.title or "No title"
                chat_type = chat.type
                member_count = chat.get_member_count() if hasattr(chat, 'get_member_count') else "Unknown"
                ...
                try:
                    ...
                    admins_text = "Cannot fetch admins (bot lacks permission)"

                notification = (
                    f"Bot was added to a new group!\n\n"
                    f"Group Title: {title}\n"
                    f"Chat ID: {chat_id}\n"
                    f"Type: {chat_type}\n"
                ...
                notification = (
                    f"Bot was added to a new group!\n\n"
                    f"Chat ID: {chat_id}\n"
                    f"Could not fetch more details: {str(e)}"
                )

            # Send detailed notification to owner in private
            ...
            # Reply in the group with your custom invite link
...
def start(message):
    ...
    if len(parts) > 1:
        param = ' '.join(parts[1:])
        broadcast_message(message.chat.id, param, is_deep_link=True)
        return
