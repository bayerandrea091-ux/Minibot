...
def bot_can_add_members(chat_id):
    ...
    try:
        ...
        if member.status == 'creator':
            return True, True  # can_add_member, can_promote
...
def bot_can_pin(chat_id):
    ...
    try:
        ...
        # Member — check if group allows members to pin
        chat = bot.get_chat(chat_id)
...
def handle_new_chat_members(message):
    ...
    for member in message.new_chat_members:
        if member.id == bot.get_me().id:
            continue
        # Check group-specific join reply first, then global
...
def group_start_command(message):
    chat_id = message.chat.id
    # Group-specific reply takes priority
...
def callback(call):
    ...
    # ── DELETE ALL MY SENT MESSAGES IN GROUP (PURGE) ──────────────────────────
    ...
    # ── ADD ACCOUNT TO GROUP (main menu) ──────────────────────────────────────
        ...
        if not eligible:
            markup = types.InlineKeyboardMarkup()
            markup.add(types.InlineKeyboardButton("🔙 Go Back", callback_data="back"))
            edit("❌ No groups where bot has permission to add members.", markup)
            answer()
            return
        ...
        markup.add(types.InlineKeyboardButton("🔙 Go Back", callback_data="back"))
        edit("➕ *Select a group to add someone to:*", markup, parse_mode='Markdown')
        answer()
    ...
    # ── ADD TO GROUP: CHOOSE ROLE ─────────────────────────────────────────────
        ...
        markup = types.InlineKeyboardMarkup(row_width=1)
        if can_promote:
            markup.add(types.InlineKeyboardButton("👑 Add as Admin", callback_data=f"add_to_group:{chat_id}:admin"))
        if can_add:
            markup.add(types.InlineKeyboardButton("👤 Add as Member", callback_data=f"add_to_group:{chat_id}:member"))
        markup.add(types.InlineKeyboardButton("🔙 Go Back", callback_data="add_account_menu"))
        edit(f"➕ *Add to:* {title}\n\nChoose role:", markup, parse_mode='Markdown')
        answer()

    # ── ADD TO GROUP: ENTER USER IDS ─────────────────────────────────────────
    elif data.startswith("add_to_group:") and (data.endswith(":admin") or data.endswith(":member")):
        parts = data.split(":")
        chat_id = int(parts[1])
        role = parts[2]
        edit(f"➕ Send the user ID(s) to add as *{role}*.\nSeparate multiple IDs with spaces or commas.", markup=None)
        bot.register_next_step_handler(call.message, lambda m: process_add_to_group(m, chat_id, role))
        answer()
...
def process_global_group_start_reply(message):
    if message.from_user.id != OWNER_ID:
        return
    text = message.text.strip()
    r.set('global_group_start_reply', text)
...
def process_group_join_reply(message, chat_id):
    ...
    bot.send_message(message.chat.id, f"✅ Join reply for group {chat_id} set.")
    show_main_menu(message.chat.id, "🏠 Choose action:")
...
def process_single_message(message, group_id):
    ...
    try:
        ...
        if r.get(f'repeat_task:{group_id}') == 'True' and not r.get(f'repeat_text:{group_id}'):
            ...
            bot.send_message(message.chat.id, "ℹ️ This message is now also the repeating message (no prior set).")
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Error: {str(e)}")
    show_main_menu(message.chat.id, "🏠 Choose action:")

def process_add_to_group(message, chat_id, role):
    if message.from_user.id != OWNER_ID:
        return
    ...
    for id_str in id_strings:
        try:
            ...
            results.append(f"❌ `{id_str}` — not a valid ID")
            continue
        try:
            if role == 'admin':
                ...
                results.append(f"✅ `{user_id}` promoted to Admin")
            else:
                bot.unban_chat_member(chat_id, user_id, only_if_banned=True)
                results.append(f"✅ `{user_id}` added as Member (unbanned if banned)")
        except telebot.apihelper.ApiTelegramException as e:
            err = str(e)
            if "USER_NOT_PARTICIPANT" in err or "user not found" in err.lower():
                results.append(f"❌ `{user_id}` — User not in group. They must join first or use invite link.")
            elif "CHAT_ADMIN_REQUIRED" in err:
                results.append(f"❌ `{user_id}` — Bot lacks admin rights.")
            elif "USER_PRIVACY_RESTRICTED" in err:
                results.append(f"❌ `{user_id}` — User privacy settings blocked this.")
            else:
                results.append(f"❌ `{user_id}` — {err}")
        except Exception as e:
            results.append(f"❌ `{user_id}` — {str(e)}")

    title, _ = get_group_info(chat_id)
    summary = f"➕ *Add Results for {title}:*\n\n" + "\n".join(results)
    markup = types.InlineKeyboardMarkup()
    markup.add(types.InlineKeyboardButton("🔙 Group Menu", callback_data=f"group_menu:{chat_id}"))
    try:
        bot.send_message(message.chat.id, summary, parse_mode='Markdown', reply_markup=markup)
    except Exception:
        bot.send_message(message.chat.id, summary.replace('`', '').replace('*', ''), reply_markup=markup)
...
if __name__ == '__main__':
    ...
    # Restart any active per-group repeat tasks (preserves existing groups)
    for g in get_groups():
        start_repeat_thread(g)

    # Restart global repeat if it was running
    start_global_repeat_thread()
