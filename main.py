                ...
                try:
                    ...
                    if '429' in err:
                        try:
                            ...
                            retry_after = 30
                        # Do NOT cap retry_after — respect exactly what Telegram says
...
def repeat_message_task(chat_id):
    while r.get(f'repeat_task:{chat_id}') == 'True':
        ...
        # Use nowait so the repeat thread never blocks waiting for send confirmation.
        # The queue worker handles delivery; the repeat thread stays on schedule.
        safe_send_nowait(chat_id, text)

        # Self-delete: wait for expected delivery + requested delay, then delete last sent
        if self_delete_after:
            secs = int(self_delete_after)
            with _send_queue_lock:
                q_depth = len(_send_queue)
            estimated_delivery = q_depth * _INTER_MSG_DELAY + 5
            def _delayed_delete(cid, delay):
                time.sleep(delay)
                last = r.get(f'last_sent:{cid}')
                if last:
                    safe_delete(cid, int(last))
            threading.Thread(target=_delayed_delete, args=(chat_id, estimated_delivery + secs), daemon=True).start()

        interval = int(r.get(f'repeat_interval:{chat_id}') or 3600)
        # Sleep for the full interval measured from cycle start (wall-clock accurate)
...
def global_repeat_task():
    while r.get('global_repeat_task') == 'True':
        ...
        for chat_id in groups:
            ...
            # Use nowait so the global repeat thread never blocks per-group.
            # All sends are queued and processed serially by the queue worker.
            safe_send_nowait(chat_id, text)
            ...
            if self_delete_after:
                ...
                def _del(cid, delay):
                    time.sleep(delay)
                    last = r.get(f'global_last_sent:{cid}')
                    if last:
                        safe_delete(cid, int(last))
                threading.Thread(target=_del, args=(chat_id, estimated_delivery + secs), daemon=True).start()

        # Interval measured from cycle start so it's wall-clock accurate regardless of send volume
        elapsed = time.time() - cycle_start
        sleep_time = max(interval - elapsed, 1)
        time.sleep(sleep_time)
...
def process_broadcast_all(message):
    ...
    if not groups:
        ...
        return
    # Queue all sends without blocking — the queue worker delivers them in order.
    # This prevents the webhook thread from freezing for minutes while waiting for confirmations.
    for group in groups:
        safe_send_nowait(group, text)
    bot.send_message(
        message.chat.id,
        f"📬 Broadcast queued for {len(groups)} groups!\n"
        f"⏱ Estimated delivery: ~{len(groups)} seconds at safe rate.\n"
        f"📡 Check Updates menu for any delivery errors.",
        reply_markup=_back_markup("back")
    )
