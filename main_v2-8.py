...
def _auto_backup_worker():
    ...
    while True:
        try:
            ...
            # ── Health check: tiered size thresholds ──────────────────────────
            # < 500B  → truly empty skeleton, Redis wiped, block + warn
            # 500B–5KB → send but note it's small (new bot / low activity)
            # > 5KB   → send normally
            _HARD_MIN = 500
            _WARN_MIN = 5120
            if buf_size < _HARD_MIN:
                logger.warning(f"[BACKUP] Truly empty backup: {buf_size} bytes — Redis wiped. NOT sending.")
                try:
                    warn_markup = types.InlineKeyboardMarkup()
                    warn_markup.add(types.InlineKeyboardButton(
                        "📤 Backup Anyway", callback_data="backup_force_send"
                    ))
                    bot.send_message(
                        OWNER_ID,
                        f"⚠️ *Backup Warning!*\n\n"
                        f"Redis appears completely empty or wiped!\n"
                        f"Backup is only *{buf_size} bytes* — just an empty skeleton.\n\n"
                        ...
                        f"_Tap below only if you're sure you want this file sent._",
                        parse_mode='Markdown',
            ...
            if now - last_sent >= 3600:
                small_note = (
                    f"\n\n⚠️ _Small backup ({buf_size} bytes). "
                    f"Could be a new bot or low activity — not necessarily a wipe._"
                    if buf_size < _WARN_MIN else ""
                )
...
def backup_command(message):
    ...
    try:
        ...
        if cached and age < 300:
            ...
            if buf_size < 500:
                ...
                bot.send_message(
                    message.chat.id,
                    f"⚠️ *Backup Warning!*\n\n"
                    f"Cached backup is only *{buf_size} bytes* — Redis appears wiped.\n\n"
                    f"❌ File NOT sent to protect your last good backup.\n"
                    f"_Tap below only if you're sure you want this file sent._",
                    parse_mode='Markdown',
                    reply_markup=warn_markup
        ...
        # < 500B  → truly empty, block + show button
        # 500B–5KB → send but add small-note to caption
        # > 5KB   → send normally
        buf_size = buf.getbuffer().nbytes
        _HARD_MIN = 500
        _WARN_MIN = 5120
...
def _process_restore_file(message):
    ...
    _edit(
        ...
        f"ℹ️ Note: Restore is additive — keys in the backup were written "
        f"but extra keys already in Redis were not removed."
    )
...
@bot.callback_query_handler(func=lambda call: call.data == 'backup_force_send')
def backup_force_send_callback(call):
    if call.from_user.id != OWNER_ID:
        return
    try:
        bot.answer_callback_query(call.id, "📤 Sending backup file…")
        with _backup_state_lock:
            buf = _last_backup_data
        if not buf:
            try:
                bot.edit_message_text(
                    "❌ No cached backup available. Run /backup to generate one.",
                    call.message.chat.id, call.message.message_id
                )
            except Exception:
                pass
            return
        def _do_force_send():
            global _last_backup_sent
            try:
                buf.seek(0)
                # Remove button immediately to prevent double-tap
                try:
                ...
                _do_send_backup_to_owner(
                    buf,
                    caption=f"📤 Forced backup — {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M')} UTC\n⚠️ Sent on your request despite small size."
                )
                with _backup_state_lock:
                    _last_backup_sent = time.time()
                try:
                ...
                logger.error(f"[BACKUP] Force-send failed: {e}")
                try:
                    bot.send_message(OWNER_ID, f"❌ Force backup send failed:\n{e}")
                except Exception:
                    pass
        threading.Thread(target=_do_force_send, daemon=True).start()
    except Exception as e:
        logger.error(f"[BACKUP] backup_force_send_callback error: {e}")
