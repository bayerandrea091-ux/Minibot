...
def callback(call):
    ...
    # ── GLOBAL /start@ GROUP REPLY ───────────────────────────────────────────
        ...
        edit(
            ...
            f"Status: {'✅ ON (overrides all groups)' if enabled else '❌ OFF'}\n\n"
            f"Current Reply:\n_{current}_\n\n"
        ...
        answer("✅ Global /start@ reply is ON — overrides all groups.", alert=True)
        ...
        r.set('global_group_start_reply_enabled', 'False')
        answer("✅ Global /start@ reply is OFF.", alert=True)
        callback(types.CallbackQuery(id=call.id, from_user=call.from_user, message=call.message, data="set_global_group_start_reply"))
        ...
        r.set('global_group_start_reply_enabled', 'False')
        answer("✅ Global group /start@ reply removed and turned OFF.", alert=True)
        callback(types.CallbackQuery(id=call.id, from_user=call.from_user, message=call.message, data="set_global_group_start_reply"))
    ...
    for id_str in id_strings:
        ...
        try:
            ...
            results.append(
                ...
                f"   • Remain Anonymous — can post as group name\n"
